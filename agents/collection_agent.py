...
class CollectionAgent(BaseAgent):
    ...
    def collect_single(self, product_name: str,
        ...
        Args:
            ...
            competitor_name: 需要重新采集的竞品名
            feedback: 针对该竞品的质检反馈

        Returns:
            CompetitorData: 该竞品的最新采集数据
        """
        self._log(f"   🔄 精细化重采: {competitor_name}")
        data = self._collect_competitor(
            product_name, product_description, competitor_name, feedback
        )
        # 更新缓存中的搜索文本
        self._log(f"   ✅ {competitor_name} 重采完成")
        return data
